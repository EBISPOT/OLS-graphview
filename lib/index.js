  ...
  var LoadingAnimation=true
    ...
    if (networkOptions.displayOptions.showInfoWindow===true)
 	...
 	function initializeIButtonBox(){
 		 ...
 		 */
 		var awesomplete=new Awesomplete(document.getElementById("ols-graphview-lookup-node-text-field"), {list:[], minChars:1, autoFirst:true});
...
return  x=tmpstring.replace("/","_");
    ...
    mainDiv.append(htmlString);
 		var container = document.getElementById('ols-graphview-vis-network');
 ...
 //Preparing the html of the Legend - which means adding the table, the relationship names, the canvas as well as the check boxes for every table row
 		...
 		relationships.forEach(function(e){
 			tmp+="<tr><td>"+e+'</td><td><canvas id="ols-graphview-canvas_'+e+'" width="30" height="15"></canvas></td><td><input type="checkbox" class="Legendoptions" name="'+e+'" id="ols-graphview-visible_'+cleanUpRelationshipLabels(e)+'" checked></td></tr>';
 		});
 ...
 //Color the root canvas
 		var c=document.getElementById("ols-graphview-canvas_root").getContext("2d");
 ...
 //After appending the legend, the colours have to be adjusted according to the colorMap
 		relationships.forEach(function(e){
 			var c=document.getElementById("ols-graphview-canvas_"+e).getContext("2d");
    ...
    if (networkOptions.dataOptions.showAllRelationships===false)
    {
      relationships.forEach(function(e){
        if (e!=="is a")
        {
          var tmpid='ols-graphview-visible_'+cleanUpRelationshipLabels(e)
      ...
      /* */
 			...
 			jQuery('.ols-graphview-visible_'+cleanUpRelationshipLabels(e)).on("click", function(event){
