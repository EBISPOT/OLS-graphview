 ...
 */
 ...
 //exports.DataSet = require('vis/lib/DataSet');   // not needed, included within network
// exports.Network = require('vis/lib/network/Network');
 ...
 module.exports = visualisation = function(){
  ...
  //this.networkOptions=jQuery.extend( true, {},this.networkOptions,opts.networkoptions);
  var mainDiv=null //jQuery("#ontology_vis");
  //console.log(networkOptions);
    ...
    htmlString+='<div id="Permaanwser" style="text-align:center;"><input id="Permalink" title="Get a static URL of this graph - so you can return or share it" class="primary" type="submit" value="Get permalink to this graph"/></div>';
  ...
  for (var i=0;i<tmpNodes.length;i++)
      {
        ...
        if (nodes.get(tmpNodes[i]).group!=="root")
        {
          ...
          /*   if (jQuery.inArray(tmpNodes[i],root)===-1)
                {              nodes.remove(tmpNodes[i]);}    */

      }

      /*
      var tmpRootNodeOptions=networkOptions.rootNode;
      var tmpOptions=jQuery.extend(true, {}, networkOptions.rootNode, networkOptions.collapsedRootNode);
      //tmpOptions["id"]=node;
      //graphdataset.nodes.update({id:node, font:{size:10, background:"white"}, shapeProperties: { borderDashes: true } });
      //graphdataset.nodes.update({id:node, font:{size:8}});
      console.log("collapsed options: ");
      ...
      */
      ...
      //graphdataset.nodes.update({id:node, collapsed: true, font:{size:8}});


      //Step 1: Remove edges that we don't need no more from Graph (optional?)
      //Step 2: Update relationshiparray (ugly! Optional?)
      //Step 3: Change status of node back to normal
      //This would be an option to really revert the stuff, but that leads to confucing problems - e.g. the possibility to produce not connecting graphs if zou revert a node in the middle and then hide it via visiability
          /****************************************
          var tmpEdges=network.getConnectedEdges(node)
          console.log(tmpEdges);
          //if there are no connectedEdges, our node is lonley in midair - so we remove it as well
          if (tmpEdges.length===0)
              {            nodes.remove(node); }

          //If there are connecting edges, we have to process them
          else
          {
          var tmpLabel=graphdataset.edges.get(tmpEdges)[0].label;
          tmpLabel=tmpLabel.substring(0,tmpLabel.length-4)

          console.log(clusterArray);

          var tmpCluster="";
          if (graphdataset.edges.get(tmpEdges[0]).to===node)
            {
              tmpCluster=graphdataset.edges.get(tmpEdges[0]).from+"to__"+tmpLabel;
            }
          else
            {
              tmpCluster=graphdataset.edges.get(tmpEdges[0]).to+"from__"+tmpLabel;
            }

            console.log("Constructed Cluster");
            console.log(tmpCluster);
            console.log(clusterArray);

            graphdataset.edges.update({id:tmpEdges[0], connectingFlag:false, label: tmpLabel, color:networkOptions.appearance.colorMap[indexof(relationships,tmpLabel)]})
            graphdataset.nodes.update({id:node, group: tmpLabel, color:networkOptions.appearance.colorMap[indexof(relationships,tmpLabel)], cluster:tmpCluster })

            //Remove the node from the root list array
            root.splice(jQuery.inArray(node, root),1)
...
if (!networkOptions.appearance.nodeShowLabel && !networkOptions.appearance.nodeShowShortId){
  console.log("Wrong option submitted! Nothing will change");
 		...
 		jQuery.getJSON(linkToGraph,function (data){
            ...
            if (jQuery.inArray(nodesTMP[0].iri, root)===-1)
              ...
              adjustLabel();
                ...
                e.group=findWhereto(edgesTMP, e.id).label;
 						...
 						root.forEach(function(e){
 							...
 							/** ******** This part is a try, don't know if we go this path  - I really don't think so*********
 							...
 							if (nodes.get(e).prevColor===undefined)
 							{
 								...
 								var previousColor=networkOptions.appearance.colorMap[indexof(relationships,nodes.get(e).group)];
              ...
              //nodes.update({id: e, group:"root", cluster: undefined, mass:networkOptions.rootNode.rootMass, color: networkOptions.rootNode.color, font: networkOptions.rootNode.font});
 					...
 					//  showNodeInfo(last(root));
 					//  network.selectNodes(last(root));
    ...
    network.on("stabilizationProgress", function(params){console.log("stabilisation!");})
    network.once("stabilizationIterationsDone", function(params) {   console.log("stabilizationDone");})
 ...
 //One side of the edge is a cluster, so we get null as result and have to change our response to that
 	...
 	function showNodeInfo(nodeId){
      ...
      //htmlString+='<input id="childrenListen" class="primary" type="submit" title="Show list of all children" value="ShowChildren"/>'
      /*****///
      ...
      /***** SO - DO we need this or is this to be deleted??? **
      jQuery("#childrenListen").on("click", function(){
          console.log("ChildrenList");
          console.log(nodeId);
          console.log(networkOptions.webservice.URL);
          console.log("Should we do this?")
      });
      /******/
      ...
      /* */
 			...
 			jQuery('#special').on("click", function(event){
 				if(jQuery('#special').prop("checked"))
 				{
 					...
 					//_.map(jQuery(".Legendoptions"), function(e){ updateDataView(e.name, false)});
          jQuery.map(jQuery(".Legendoptions"), function(e){ updateDataView(e.name, false)});
 				...
 				else
 				{
 					...
 					//_.map(jQuery(".Legendoptions"), function(e){updateDataView(e.name, true)});
