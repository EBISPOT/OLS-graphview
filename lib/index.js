 ...
 module.exports = visualisation = function(){
  ...
  //console.log(networkOptions);
 	...
 	function initializeIButtonBox(){
 		console.log("initializeButtonBox");
...
if (root.length>0)
  ...
  for (var i=0;i<tmpNodes.length;i++)
      ...
      */
      ...
      graphdataset.nodes.update({id:node, font:{size:8}});
      //Remove the node from the root list array // It is an easy work around ,even though not totally correct
      root.splice(jQuery.inArray(node, root),1)
          ...
          //This would be an option to really revert the stuff, but that leads to confucing problems
          /****************************************
 	...
 	function fetchGraphData(linkToGraph){
 		...
 		jQuery.getJSON(linkToGraph,function (data){
 					...
 					if (data.nodes.length!=0 && data.edges.length!=0)
 					{
 						...
 						//Remember the root node for later fixing of groups
              ...
              //Try to implement different form of options. Failed misserably
 ...
 //  var id=nodeId // DO WE HAVE TO ENCODE THIS?
 		...
 		jQuery.getJSON(networkOptions.webservice.URL+id, function(inputdata){
 			...
 			jQuery("#infoWindow").html("<h6>Selected Node - "+data.label+"</h6>");
 ...
 //Finally, add also a Listener to the Permalink Button
 		jQuery("#Permalink").on("click", function()
 		{
 			...
 			jQuery("#Permaanwser").html('<style="text-align:center; margin-top:100px;"><hr style="margin-left: 50px; margin-right:50px"/><input type="text" title="Copy and paste this link to return to this graph! You can easly share it via email as well!" name="Permalink" value="'+rootNodes+'">')
 		});
